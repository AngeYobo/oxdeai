...
import { proposeCallsViaOpenClaw } from "./adapter.js";

export async function runDemo(log: (msg: string) => void = (msg) => console.log(msg)): Promise<void> {
  const decisions: string[] = [];

  log("╔══════════════════════════════════════════════════════════════════╗");
  log("║  OxDeAI — OpenClaw Integration Boundary Demo                    ║");
  ...
  log("Source:  tool proposals from OpenClaw runtime flow");
  ...
  const plannedCalls = await proposeCallsViaOpenClaw(log);

  log("\n── Agent proposals (from OpenClaw) ────────────────────────────────");
