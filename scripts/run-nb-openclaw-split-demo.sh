...
tmux send-keys -t "$SESSION":0.2 "export UPSTREAM_EXECUTOR_TOKEN=$TOKEN; node examples/non-bypassable-demo/protected-upstream.mjs" C-m
sleep 1.5

tmux send-keys -t "$SESSION":0.1 "export UPSTREAM_EXECUTOR_TOKEN=$TOKEN; node examples/non-bypassable-demo/pep-gateway.mjs" C-m
sleep 2

tmux send-keys -t "$SESSION":0.0 "clear; pnpm -C examples/non-bypassable-openclaw agent" C-m
